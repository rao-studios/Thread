...
func wipeThreadPersistenceFiles() {
...
extension Logger {
    static var test: Logger {
        var logger = Logger(label: "test-thread")
        logger.logLevel = .critical
        return logger
    }
}

extension ThreadLogger {
    static var test: ThreadLogger { ThreadLogger(.test) }
}
