...
final class Flow20_BatchEmbeddingsRaceTests: XCTestCase {
    ...
    /// Thread has no auth middleware — ownerId comes directly from the request body.
    private func makeApp(database: Database) -> Application<RouterResponder<ThreadRequestContext>> {
