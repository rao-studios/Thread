...
//    ThreadCache<Value>  — lock-protected snapshot + off-actor persistence
//
...
final class Flow5_BatchIndexingTests: XCTestCase {
    ...
    func testRegisterBatchRegistersAllDocuments() async {
        ...
        let owner = ThreadRegistry.Owner(id: "batch-owner")
    ...
    func testPersistenceActorSaveAndRestoreRoundtrip() async {
        ...
        registry.documentOwners["sentinel"] = [ThreadRegistry.Owner(id: "actor-test")]
        await actor.save(registry)

        let restored: ThreadRegistry? = await actor.restore()
    ...
    func testPersistenceActorConcurrentSavesProduceValidFile() async {
        ...
        await withTaskGroup(of: Void.self) { group in
            for i in 0..<20 {
                var registry = ThreadRegistry()
                registry.documentOwners["doc-\(i)"] = [ThreadRegistry.Owner(id: "owner-\(i)")]
    ...
    func testPersistenceActorPurgeRemovesFile() async {
        ...
        let before: ThreadRegistry? = await actor.restore()
        XCTAssertNotNil(before, "File must exist after save")

        await actor.purge()

        let after: ThreadRegistry? = await actor.restore()
        XCTAssertNil(after, "restore() must return nil after purge()")
    }

    // MARK: - ThreadCache

    func testThreadCacheSnapshotIsNilInitially() {
        let cache = ThreadCache<ThreadRegistry>(persistence: tempPersistence(key: "sc-nil"))
        XCTAssertNil(cache.snapshot)
    }

    func testThreadCacheSeedSetsSnapshotSynchronously() {
        let cache = ThreadCache<ThreadRegistry>(persistence: tempPersistence(key: "sc-seed"))
    ...
    func testThreadCacheUpdateReflectsInSnapshot() {
        let cache = ThreadCache<ThreadRegistry>(persistence: tempPersistence(key: "sc-update"))
        cache.seed(.init())

        var updated = ThreadRegistry()
    ...
    func testThreadCacheUpdateReplacesEntireSnapshot() {
        let cache = ThreadCache<ThreadRegistry>(persistence: tempPersistence(key: "sc-replace"))
        cache.seed(.init())

        var r1 = ThreadRegistry()
        r1.documentOwners["a"] = [.init(id: "owner-a")]
        cache.update(r1)
        XCTAssertNotNil(cache.snapshot?.documentOwners["a"])

        var r2 = ThreadRegistry()
    ...
    func testThreadCacheLoadReturnsSeededValueWithoutDiskRead() async {
        let cache = ThreadCache<ThreadRegistry>(persistence: tempPersistence(key: "sc-load-seeded"))

        var seeded = ThreadRegistry()
        ...
        let result = await cache.load { ThreadRegistry() }
    ...
    func testThreadCacheSnapshotUpdatedBeforeDiskFlush() {
        let cache = ThreadCache<ThreadRegistry>(persistence: tempPersistence(key: "sc-immediate"))
        cache.seed(.init())
    ...
    func testThreadCacheSaveAsyncThenLoadOnFreshCacheReturnsPersistedValue() async throws {
        let key = "sc-save-async-\(UUID().uuidString)"
        let writer = ThreadCache<ThreadRegistry>(persistence: tempPersistence(key: key))
        writer.seed(.init())
        ...
        let reader = ThreadCache<ThreadRegistry>(persistence: tempPersistence(key: key))
        let loaded = await reader.load { ThreadRegistry() }
    ...
    func testThreadCacheLoadPopulatesSnapshotForSubsequentReads() async {
        ...
        _ = await cache.load { ThreadRegistry() }
